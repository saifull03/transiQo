    ...
    const fetchHistory = async () => {
      try {
        ...
        const { data } = await axios.get(
          "http://localhost:5003/api/rides/history",
          config,
        );
        setRides(data);
      } catch (error) {
        ...
        {rides.length === 0 ? (
          ...
          <div className="space-y-4">
            {rides.map((ride) => (
              <div
                key={ride._id}
                className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-md border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between"
              >
